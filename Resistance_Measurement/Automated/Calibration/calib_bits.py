...
test_res_1 = {'01':150.3, '02':150.2, '03':150.0, '04':150.5, '05':150.1,   
              '06':150.1, '07':150.0, '08':150.2, '09':150.3, '10':151.1,   
              '11':149.5, '12':150.5, '13':149.9, '14':150.0, '15':150.1,
              '16':149.9, '17':149.6, '18':150.3, '19':150.3, '20':149.9, 
              '21':149.6, '22':149.8, '23':150.0, '24':150.6}

test_res_2 = {'01':0.1, '02':0.1, '03':0.1, '04':0.1, '05':0.1,
#test_res_2 = {'01':75, '02':0, '03':0, '04':0, '05':0,
              '06':0.1, '07':0.1, '08':0.1, '09':0.1, '10':0.1,
...
board_meas = [8.0,5.0,509.8,509.8,510.0,510.0,509.8,510.0]
...
calib_file = 'bits_calib_check_' + datetime.now().strftime('%Y-%m-%d_%H%M%S') + '.csv'
...
def arduino_voltage(v5, first_straw):
    ...
    for key, value in sorted(first_straw.items()):
        straw1 = int(value[:2])
        measuring = value[2:]
        straws = [str('%02d'%(straw1))+measuring, str('%02d'%(straw1+4))+measuring,
                  str('%02d'%(straw1+8))+measuring, str('%02d'%(straw1+12))+measuring,
                  str('%02d'%(straw1+16))+measuring, str('%02d'%(straw1+20))+measuring]
        ser.write(bytes(key+'r', 'ascii')) #send signal to switch to new cycle, then read
...
def calculate_calib(meas_dict, tr1, tr2, straw_nums): #tr = test resistor
    ...
    for key, value in sorted(meas_dict.items()):
        ...
        if (0.10 <= calib_dict[key][0] <= 0.40) and (100.0 <= calib_dict[key][1] <= 500):  
...
def display_calib(vin, v5, r2_list, save_dict):
    ...
    print('R0 = ' + str(r2_list[0]) + ', ' + 'R1 = ' + str(r2_list[1]) + ', '
         +'R2 = ' + str(r2_list[2]) + '\n' + 'R3 = ' + str(r2_list[3]) + ', '
         +'R4 = ' + str(r2_list[4]) + ', ' + 'R5 = ' + str(r2_list[5]) + '\n\n')  
    
    i = 1
    print('#/Type %Err     Dev_Res')
    print('-----------------------')
    for key, value in sorted(save_dict.items()):
        print('(' + key + ': ' + '%5.3f'%value[0] + ', ' + '%8.2f'%value[1] + ', ', end='')
        if value[2] == 'pass':
...
def save_calib(c_file, vin, v5, r2, calib_dict):
    ...
    c_file.write(str(r2[0]) + ', ' + str(r2[1]) + ', ' + str(r2[2]) + ', '
                +str(r2[3]) + ', ' + str(r2[4]) + ', ' + str(r2[5]) + '\n')
...
def main():
    ...
    with open(calib_file, 'w') as f:
        ...
        if use_store == 'y':
            r2_vals = [0,0,0,0,0,0]
            vin, v5, r2_vals[0], r2_vals[1], r2_vals[2], r2_vals[3], r2_vals[4], r2_vals[5] = board_meas
        else:
        ...
        save_dict = {}
        for value in straw_nums:
            save_dict[value+'ii'] = [0,0,'fail']  #[% error, device resistance, pass/fail]
            save_dict[value+'io'] = [0,0,'fail']  #[% error, device resistance, pass/fail]
            save_dict[value+'oi'] = [0,0,'fail']  #[% error, device resistance, pass/fail]
            save_dict[value+'oo'] = [0,0,'fail']  #[% error, device resistance, pass/fail]
