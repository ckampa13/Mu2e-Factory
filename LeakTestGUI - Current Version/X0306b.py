...
#### Goal3: Avoid freezing GUI to wait for serial data
...
class LeakTestStatus(QMainWindow):
    def __init__(self,COM,baudrate,arduino_input=False):
        ...
        self.COM = ['COM11','COM7','COM9','COM12','COM6','COM10','COM17','COM14','COM15','COM16']
        self.COM_con = [None,None,None,None,None,None,None,None,None,None]
        self.arduino = [serial.Serial(port=self.COM_con[0],baudrate=115200,timeout=0.08),serial.Serial(port=self.COM_con[1],baudrate=115200,timeout=0.08),serial.Serial(port=self.COM_con[2],baudrate=115200,timeout=0.08),
                        serial.Serial(port=self.COM_con[3],baudrate=115200,timeout=0.08),serial.Serial(port=self.COM_con[4],baudrate=115200,timeout=0.08),serial.Serial(port=self.COM_con[5],baudrate=115200,timeout=0.08),
                        serial.Serial(port=self.COM_con[6],baudrate=115200,timeout=0.08),serial.Serial(port=self.COM_con[7],baudrate=115200,timeout=0.08),serial.Serial(port=self.COM_con[8],baudrate=115200,timeout=0.08),
                        serial.Serial(port=self.COM_con[9],baudrate=115200,timeout=0.08)]
        
        self.baudrate = [115200,115200,115200,115200,115200,115200,115200,115200,115200,115200]
...
#            x.setDisabled(True)
    ...
    def Connect_Arduino(self, a_port):
        ...
        self.arduino[index] = serial.Serial(port=self.COM_con[index],baudrate=115200,timeout=0.08)
        
    def startArduino1(self):
        self.arduino[0].close()
        self.Connect_Arduino(self.COM[0])
        if self.COM_con[0] == None:
           self.Arduinos[0].setStyleSheet("background-color: rgb(170, 0, 0);")
           return
    ...
    def startArduino2(self):
        self.arduino[1].close()
        self.Connect_Arduino(self.COM[1])
        if self.COM_con[1] == None:
           self.Arduinos[1].setStyleSheet("background-color: rgb(170, 0, 0);")
           return
    ...
    def startArduino3(self):
        self.arduino[2].close()
        self.Connect_Arduino(self.COM[2])
        if self.COM_con[2] == None:
           self.Arduinos[2].setStyleSheet("background-color: rgb(170, 0, 0);")
           return
    ...
    def startArduino4(self):
        self.arduino[3].close()
        self.Connect_Arduino(self.COM[3])
        if self.COM_con[3] == None:
           self.Arduinos[3].setStyleSheet("background-color: rgb(170, 0, 0);")
           return
    ...
    def startArduino5(self):
        self.arduino[4].close()
        self.Connect_Arduino(self.COM[4])
        if self.COM_con[4] == None:
           self.Arduinos[4].setStyleSheet("background-color: rgb(170, 0, 0);")
           return
    ...
    def startArduino6(self):
        self.arduino[5].close()
        self.Connect_Arduino(self.COM[5])
        if self.COM_con[5] == None:
           self.Arduinos[5].setStyleSheet("background-color: rgb(170, 0, 0);")
           return
    ...
    def startArduino7(self):
        self.arduino[6].close()
        self.Connect_Arduino(self.COM[6])
        if self.COM_con[6] == None:
           self.Arduinos[6].setStyleSheet("background-color: rgb(170, 0, 0);")
           return
    ...
    def startArduino8(self):
        self.arduino[7].close()
        self.Connect_Arduino(self.COM[7])
        if self.COM_con[7] == None:
           self.Arduinos[7].setStyleSheet("background-color: rgb(170, 0, 0);")
           return
    ...
    def startArduino9(self):
        self.arduino[8].close()
        self.Connect_Arduino(self.COM[8])
        if self.COM_con[8] == None:
           self.Arduinos[8].setStyleSheet("background-color: rgb(170, 0, 0);")
           return
    ...
    def startArduino10(self):
        self.arduino[9].close()
        self.Connect_Arduino(self.COM[9])
        if self.COM_con[9] == None:
           self.Arduinos[9].setStyleSheet("background-color: rgb(170, 0, 0);")
           return
    ...
    def ReadinBuffer(self,ROW):
        ...
        while starting_up:
            ...
            if (time.time() - begining_time) > starting_up_time:
                starting_up = False
                ...
                if tf == True:
                    ...
                    if arduino_line != b'':
                        ...
                        with open(self.files[file],'a+',1) as f:
                            f.write(str(format(epoctime[ROW],'.0f'))+'\t'+str(file)+'\t'
                                    +("%.0f" % float(formattedList[1]))+'\t'+str(currenttime)+'\n')
    ...
    #                            starttime[chamber] = 0
                                ...
                                with open(self.directory + self.Choosenames[ROW][COL] + '_rawdata.txt',"r+",1) as readfile :
                                    ...
                                    print('# ppm length is is '+str(len(PPM[chamber]))+ 'for chamber %.0f' % chamber)
                                    print('# ppm_err length is is '+str(len(PPM_err[chamber]))+ 'for chamber %.0f' % chamber)
                                    print('# timestamp length is is '+str(len(timestamp[chamber]))+ 'for chamber %.0f' % chamber)
                                    ######################################################################################################
...
#                    x.setDisabled(True)
    ...
    def ChangeStraw(self,chamber):
        ...
        if straw == "empty" : # Empty chamber
                ...
                self.Choosenames[ROW][COL] = straw + '_chamber%.0f_' % chamber+datetime.now().strftime("%Y_%m_%d")
...
class StrawSelect(QDialog):
    ...
    def StrawInput(self):
        ...
        if True:
            #*****re-enable if statement below for production******
