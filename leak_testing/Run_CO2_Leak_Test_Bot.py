...
#calibration volumes from pure CO2 on 9/1/2016
chamber_volume = [533,489,499,500,422]
chamber_volume_err = [16,12,12,50,19]
...
#name of chambers
chamber_id = { "ch5" : 0, "ch6" : 1, "ch7" : 2, "ch8" : 3, "ch9" : 4, 0 : "ch5", 1 : "ch6", 2 : "ch7", 3 : "ch8", 4 : "ch9"} 
Choosenames = [ "empty5", "empty6", "empty7", "empty8", "empty9" ]
files = {}
#create a csv file to store leak test results
result = open('C:\\Users\\vold\\Desktop\\Leak Test Results\\' + datetime.now().strftime("%Y-%m-%d_%H%M%S") + '_COM7.csv', "a+",1)
...
def Change_straws() :
        ...
        if straw == "empty" : #empty chamber
                ...
                Choosenames[this_chamber] = straw + '_chamber%.0f_' % (this_chamber + 5) + datetime.now().strftime("%Y_%m_%d") 
        print(straw)
        Need_for_change()
